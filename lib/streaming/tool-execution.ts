...
// Deserialize the pendle schemas to string
const pendleOpportunitiesSchemaString = JSON.stringify(pendleOpportunitiesTool.parameters, null, 2)
...
export async function executeToolCall(
  ...
  const toolSelectionResponse = await generateText({
    ...
    system: `You are an intelligent assistant that analyzes conversations to select the most appropriate tools and their parameters.
            ...
            - pendle_quote: Use when the user wants to know the conversion rate between ETH and a specific Pendle market token (PT or YT).
            - pendle_swap: Use when the user wants to actually execute a swap of ETH to a specific Pendle market token (PT or YT). Only use this when the user explicitly requests to make a transaction.
            ...
            ${pendleOpportunitiesSchemaString}
  ...
  // If not search, try pendle opportunities tool
  ...
  // If not pendle swap, try wallet balance tool
  if (!toolName || toolName === '') {
  ...
  if (toolName === 'search') {
    ...
    // Type guard for pendle opportunities tool: must NOT have 'query', but have at least one of the pendle params
    if (
      toolParams &&
      typeof toolParams === 'object' &&
      !('query' in toolParams) &&
    ...
    if (
      ...
      // Apply default value for slippage if not provided
      const swapParams = {
        ...toolParams,
        slippage: toolParams.slippage ?? 0.01 // Default to 1% slippage if not specified
      } as { 
        market_address: string; 
        token_out_address: string; 
  ...
  if (toolName === 'pendle_opportunities' || toolName === 'wallet_balance' || toolName === 'pendle_quote' || toolName === 'pendle_swap') {
